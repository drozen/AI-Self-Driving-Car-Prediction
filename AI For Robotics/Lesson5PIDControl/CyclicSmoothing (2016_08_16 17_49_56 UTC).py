    ...
    while change >= tolerance:
        ...
        for i in range(len(path)):
            for j in range(len(path[0])):
                #auxilary variable
                aux = newpath[i%len(path)][j]                      
                newpath[i%len(path)][j] += a * (path[i%len(path)][j] - newpath[i%len(path)][j]) + b * (newpath[i%len(path)-1][j] + newpath[i%len(path)+1][j] - (2.0 * newpath[i%len(path)][j]))
                # increment change
                change += abs(aux - newpath[i%len(path)][j])
