...
class robot:
    ...
    def measurement_prob(self, measurements):
        ...
        predicted_measurements = self.sense(0) # Our sense function took 0 as an argument (or parameter) to switch off noise.  This is a flag
        # When set to 0 it switches off the noise modeling and gets you the predicted best possible measurements.



        # and then I compute a Gaussian that measures the distance between
        # the measurements passed into the routine 
        # and the predicted measurements  "measurements"
    ...
    def move(self, motion, tolerance = 0.001): # Do not change the name of this function
        ...
        ## motion vector is [ steering angle, robot motion distance]
        ...
        if abs(turn) < tolerance:
            ...
            # centre of circle I'm turning 
            cx = self.x - (sin(self.orientation) * radius)
            cy = self.y + (cos(self.orientation) * radius)
            
            # change orientation to be the new orientation by adding turn
            res.orientation = (self.orientation + turn) % (2.0 * pi)
            
            # then plug in new orientation
...
##
motions = [[2. * pi / 10, 20.] for row in range(4)]
